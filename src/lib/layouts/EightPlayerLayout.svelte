...
<div class="flex flex-col gap-0 h-full justify-between" data-dnd-layout>
	<div class="flex flex-col gap-2 bg-black sticky top-0" class:h-[18%]={!$appState.isMenuOpen}>
	...
	{#if !$appState.isMenuOpen}
		<div class="flex gap-2 bg-black sticky top-0 pb-2" class:h-[20%]={!$appState.isMenuOpen}>
			...
			<div class="w-1/2" class:h-full={!$appState.isMenuOpen}>
				<DraggablePlayerSlot className="w-full h-full" seatId={8}>
					<PlayerHorizontal
						doNotShowMinimap={false}
						orientation="left"
						layout="one-two-one"
	...
	<div class="flex flex-col gap-2 bg-black sticky bottom-0" class:h-[18%]={!$appState.isMenuOpen}>
		<div class="h-full w-full flex justify-between gap-2">
			<div class="w-full">
