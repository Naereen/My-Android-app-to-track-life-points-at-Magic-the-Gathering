...
export const searchGifs = async (q: string): Promise<KlipyResult[]> => {
    ...
    try {
        ...
        const results = (raw as any[]).map((it: any) => ({
            ...
            preview: it?.blur_preview ?? null,
            url: it?.file?.md?.webp?.url ?? it?.file?.md?.gif?.url ?? it?.file?.md?.jpg?.url ?? it?.file?.md?.mp4?.url
                ?? it?.file?.hd?.webp?.url ?? it?.file?.hd?.gif?.url ?? it?.file?.hd?.jpg?.url ?? it?.file?.hd?.mp4?.url
