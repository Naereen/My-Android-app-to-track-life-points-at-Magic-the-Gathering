...
- [x] Write a `manifest.json` for PWA support
  ...
  - [x] Test installing the PWA on Apple devices (iOS/iPadOS) - TODO: I need an Apple device for that! I tested on an iPhone of a friend, and they don't support installation of PWA, this settles that!
...
- [x] There is a bug in the ServiceWorker.js script, which is generated by the Vite PWA plugin, but I don't know how to fix it yet. It doesn't seem to cause any issue for the app, but it is still a bug that should be fixed.
...
- [x] Commander damage type tracking, in a simple and visual way, with a small icon for each opponent, and a number for the commander damage received from this opponent. It is a new feature from the Lifetap app, and it is very useful for Commander games!
  ...
  - [ ] When clicking on the Minimap, I want to directly open the playerModal on the "EDH Damage" section. This is a very useful feature, as it allows to quickly edit the commander damage received from each opponent, without having to open the player modal and navigate to the "Status effects" tab, which can be long and boring, especially on mobile devices.
...
### Stream view: client/server architecture for streaming the game state to a live stream (OBS then streaming to Twitch, YouTube, etc.)

- [ ] If there are only two players, the fontsize of the life total of the two players should be much larger! This can be useful for streaming, as it allows the viewers to easily see the life total of each player, even on small screens (e.g. mobile devices).

- [ ] Add a "streaming mode" in the settings, where the app is a client OR a server (I'm still unsure which of the two approach would be better), that can stream the game state (life total, commander damage, poison counters, status effects, etc.) to a live stream (e.g. OBS), so that it can be displayed on Twitch, YouTube, etc.
  - [ ] This can be a very cool feature for streamers who want to share their duel games with their audience, as it allows them to easily display the game state in a visually appealing way, without having to use a separate overlay or software.
  - [ ] It can also be useful for players who want to record their games and share them later, as it allows them to have a nice visual representation of the game state in their recordings.
...
- [ ] Multiplayer online mode (connect with friends over the internet), à la Mythic.Tools?
- [ ] Customizable themes and UI options, à la Mythic.Tools
